...
void Delay_us(uint32_t time_delay)
	{
		...
		while(time_delay--)
			{
				while(__HAL_TIM_GET_FLAG(&htim4,TIM_FLAG_UPDATE)== 0) ;				// cho co tran = 0 de thoat khoi vong lap
...
static void ssd1306_write_byte(uint8_t chData, uint8_t chCmd) 
{
	...
	data[1] = chData;
	type[0] = chCmd;
	if(type[0]) {data[0] = 0x40;}
	else {data[0] = 0x00;}
	while(HAL_I2C_Master_Transmit(&hi2c2,OLED_ADDRESS,data,2,2)!=HAL_OK);
